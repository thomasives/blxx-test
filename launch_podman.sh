...
echo podman run -it --network host $@ -v=${ioc}/config:/epics/ioc/config:Z ${image} ${command}
podman run -it --network host $@ -v=${ioc}/config:/epics/ioc/config:Z -vautosave:/autosave ${image} ${command}
